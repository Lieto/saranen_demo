...
server <- function(input, output, session) {
  ...
  ratingByPlatform <- reactive({
    if (input$selectedPlatform == 'All') {
      dat <- data.frame(
        ...
        CriticRating = ratings$CriticRating)
  ...
  output$ratingsPlot <- renderChart3({
    ...
    np$layer(
      y = 'Criticrating', x='UserRating', data=pred, copy_layer=T, type='point', color=list(const='black'), size = list(const=5)
    )
    return(np)
  ...
  output$prediction <- renderText(paste('Predicted critic rating: ', round(pred(),2), sep = ''))
